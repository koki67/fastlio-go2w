...
def test_rejects_unknown_profile_before_ros_preflight(tmp_path):
    completed = run(make_bag(tmp_path), "--profile", "fused-experimental")
    assert completed.returncode != 0
    assert "unknown profile" in completed.stderr
