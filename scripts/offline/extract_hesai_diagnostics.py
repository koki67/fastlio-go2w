#!/usr/bin/env python3
"""Extract the final Hesai adapter DiagnosticArray from an offline result bag."""
...
INTEGER_KEYS = {
    ...
    "dropped_timestamp_out_of_range",
    "dropped_too_few_points",
    "invalid_points",
}
FLOAT_KEYS = {"latest_scan_width_sec", "lidar_time_offset_sec"}
...
def diagnostic_document(
    ...
    *,
    ...
    statuses = [
        ...
        if status.name == "fastlio_go2w_hesai/pointcloud_adapter"
    ]
    if len(statuses) != 1:
        raise ValueError(
            "final DiagnosticArray must contain exactly one Hesai adapter status"
        )
    status = statuses[0]
...
def extract(result_bag: Path, topic: str) -> dict[str, Any]:
    ...
    if topic_types[topic] != "diagnostic_msgs/msg/DiagnosticArray":
        raise ValueError(
            f"unexpected diagnostics type {topic_types[topic]!r} on {topic}"
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--topic",
        default="/fastlio_go2w_hesai/diagnostics",
    )
    return parser
