...
PUBLISHER="$SCRIPT_DIR/publish_multilidar_artifacts.py"
RVIZ_CONFIG="$SCRIPT_DIR/rviz/offline_result.rviz"

usage() {
    cat <<'EOF'
Usage:
  scripts/offline/visualize_multilidar_run.sh RUN_DIR [options]
...
Dynamic mode additionally replays the already-computed result bag; it never
runs FAST-LIO or the fusion node again.
EOF
}
